...
// Minimal typings to avoid dependency on keycloak-js types
...
function getKeycloakConfig() {
  ...
  if (!url || !realm || !clientId) {
    // Provide placeholders to avoid runtime crashes and log a warning
    // These must be configured by the integrator
    // eslint-disable-next-line no-console
    console.warn('[Auth] Keycloak config missing. Set REACT_APP_KEYCLOAK_URL, REACT_APP_KEYCLOAK_REALM, REACT_APP_KEYCLOAK_CLIENT_ID');
  }
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // keycloak-js loaded via global script tag in public/index.html as window.Keycloak
    const anyWindow = window as any;
    if (!anyWindow.Keycloak) {
      console.warn('[Auth] Keycloak script not found. Add CDN script to public/index.html.');
      setInitialized(true);
      return;
    }
    ...
    const kc: KeycloakInstance = anyWindow.Keycloak({
      url: cfg.url,
      realm: cfg.realm,
    ...
    kc
      .init({ onLoad: 'check-sso', checkLoginIframe: false, silentCheckSsoRedirectUri: undefined })
      .then((auth: boolean) => {
